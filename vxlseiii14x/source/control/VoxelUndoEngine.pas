...
Procedure ResetUndo(var _Undo_Redo : TUndo_Redo);
...
begin
   ...
   if _Undo_Redo.Data_no > Max_Undos then
...
var
   i: integer;
begin
   for i := Low(_Undo_Redo.Data) to High(_Undo_Redo.Data) do
   begin
      SetLength(_Undo_Redo.Data[i].Data, 0);
   end;
   SetLength(_Undo_Redo.Data, 0);
   _Undo_Redo.Data_no := 0;
end;
...
begin
   ...
   _Undo_Redo.Data[_Undo_Redo.Data_no-1].Data_no := 0;
   SetLength(_Undo_Redo.Data[_Undo_Redo.Data_no-1].Data, _VXL.Tailer.XSize * _VXL.Tailer.YSize * _VXL.Tailer.ZSize);
...
var
   ...
   v : TVoxelUnpacked;
begin
   if (_VXL.Tailer.XSize <> _Undo_Redo.Data[_Undo_Redo.Data_no-1].XSize) or (_VXL.Tailer.YSize <> _Undo_Redo.Data[_Undo_Redo.Data_no-1].YSize) or (_VXL.Tailer.ZSize <> _Undo_Redo.Data[_Undo_Redo.Data_no-1].ZSize) then
   begin
      _VXL.Resize(_Undo_Redo.Data[_Undo_Redo.Data_no-1].XSize, _Undo_Redo.Data[_Undo_Redo.Data_no-1].YSize, _Undo_Redo.Data[_Undo_Redo.Data_no-1].ZSize);
   end;
   ...
   begin
      ...
      _VXL.SetVoxel(_Undo_Redo.Data[_Undo_Redo.Data_no-1].Data[i].Pos.X,_Undo_Redo.Data[_Undo_Redo.Data_no-1].Data[i].Pos.Y,_Undo_Redo.Data[_Undo_Redo.Data_no-1].Data[i].Pos.Z, v);
   end;
...
var
   I,J : integer;
begin
   _Dest.Data_no := _Source.Data_no-1;
   SetLength(_Dest.Data,_Dest.Data_no);
   for i := 1 to _Source.Data_no-1 do
   begin
      _Dest.Data[i-1].XSize := _Source.Data[i].XSize;
      _Dest.Data[i-1].YSize := _Source.Data[i].YSize;
      _Dest.Data[i-1].ZSize := _Source.Data[i].ZSize;
      _Dest.Data[i-1].Data_no := _Source.Data[i].Data_no;
      SetLength(_Dest.Data[i-1].Data,_Dest.Data[i-1].Data_no);
      For j := 0 to _Source.Data[i].Data_no-1 do
      begin
         _Dest.Data[i-1].Data[j].Pos.X := _Source.Data[i].Data[j].Pos.X;
         _Dest.Data[i-1].Data[j].Pos.Y := _Source.Data[i].Data[j].Pos.Y;
         _Dest.Data[i-1].Data[j].Pos.Z := _Source.Data[i].Data[j].Pos.Z;

         _Dest.Data[i-1].Data[j].V.Colour := _Source.Data[i].Data[j].V.Colour;
         _Dest.Data[i-1].Data[j].V.Normal := _Source.Data[i].Data[j].V.Normal;
         _Dest.Data[i-1].Data[j].V.Flags := _Source.Data[i].Data[j].V.Flags;
         _Dest.Data[i-1].Data[j].V.Used := _Source.Data[i].Data[j].V.Used;
      end;
   end;
end;

// Should reduce the undo making it have Max_Undos
Procedure GoneOverReset(Var _Undo_Redo : TUndo_Redo);
Var
   UndoT : TUndo_Redo;
begin
